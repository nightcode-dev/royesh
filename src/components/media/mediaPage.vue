<template>
	<div class="flex lg:flex-row flex-col-reverse w-full pt-1 h-full">
		<div
			class="lg:w-3/6 border-t-2 lg:border-t-0 lg:border-l-2 flex flex-col border-gray-200 p-4 bg-gray-100"
		>
			<div class="flex flex-col items-center">
				<div
					...
					class="w-full text-center p-4 mb-2 backdrop-blur-sm backdrop-grayscale w-2/4"
				>
				...
				<select
					id="fayde"
					class="border-2 rounded-md font-mikh w-3/6 outline-none mb-2"
				>
					<option v-for="(text, tid) in fopt" :key="tid" :value="text">
						{{ text }}
					</option>
				</select>
				...
				<div
					...
					id="descinp"
					class="bg-white w-3/4 mb-2 outline-none border-green-500 border-2 font-mikh p-1 mt-2 rounded-md"
				></div>
				<button
					@click="subCom"
					class="font-mikh font-bold p-1 rounded-md border-t-0 text-center text-white flex justify-center border-2 bg-green-500 border-green-500"
				>
					<svg
  ...
  <path stroke-linecap="round" stroke-linejoin="round" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
    ...
    <p class='text-right  font-mikh'>{{ comment.descy }}</p>
		...
		<div class="lg:w-3/6 flex flex-col items-center p-4">
			<h1 class="text-green-500 font-Bmikh text-xl mb-2">
				{{ media[0].title }}
			</h1>
			<div class="font-mikh">
				<video width="520" height="240" :poster="media[0].cover_link" controls>
					<source :src="media[0].v_link" type="video/mp4" />
					مرورگر شما ویدیو پلیر را پشتیبانی نمی کند
				</video>
			</div>
			<div class="flex items-center mt-4 mb-4">
				<div
					class="flex p-1.5 text-center font-bold text-sm items-center font-mikh text-gray-400 ml-2 bg-gray-100 rounded-md"
				>
					<svg
						xmlns="http://www.w3.org/2000/svg"
						class="h-6 w-6"
				...
				>
					...
					<p class="mr-1">{{ media[0].author }}</p>
				</div>
			</div>
			<p class="w-5/6 font-mikh text-right">{{ media[0].descy }}</p>
		</div>
	</div>
</template>
...
export default {
	setup() {
      ...
      axios.post('https://api.nightcode.ml/api/media',null,{
            params:{
              method:'comments',
              data:JSON.stringify({
                "id":route.params.id
	...
	function subCom() {
      ...
      .then(resp =>{
		...
		getMed();
		return { notftext, nothide, inserted, fopt, media,comments,subCom };
	},
};
</script>
