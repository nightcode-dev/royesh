<template>
	...
	>
		...
		<h1 class="font-Dmikh text-right text-green-500 text-xl">افزودن محصول</h1>
		<div class="flex w-full flex-col items-start font-mikh mt-2">
			<label for="qtitle">عنوان محصول</label>
			<input
				type="text"
				id="titleinp"
		...
		<div class="flex w-full flex-col items-start font-mikh mt-2">
			<label for="qtitle">توضیح محصول</label>
			<div
		...
		<div class="flex w-full flex-col items-start font-mikh mt-2">
			<label for="qtitle">قیمت محصول</label>
			<input
				type="number"
				id="priceinp"
				class="bg-white w-3/4 outline-none border-green-500 border-2 font-mikh p-1 mt-1 rounded-md hover:bg-gray-100"
		...
		<div class="flex w-full flex-col items-start font-mikh mt-2">
			<label for="qtitle">شماره تماس شما</label>
			<input
				type="number"
				id="telinp"
				class="bg-white w-3/4 outline-none border-green-500 border-2 font-mikh p-1 mt-1 rounded-md hover:bg-gray-100"
				placeholder="شماره تماس خود را جهت ارتباط با مشتری وارد کنید مثل 09876543210"
			/>
		</div>
		<div class="flex w-full flex-col items-start font-mikh mt-2">
			<label for="qtitle">عکس محصول</label>
		...
		<button
			@click="addPro"
			class="border-2 w-40 font-mikh border-green-500 p-1 mt-4 rounded-md bg-green-500 text-white flex justify-center"
		>
			<svg
			...
			<span class="mr-1">ثبت محصول</span>
		</button>
	</div>
</template>
...
export default {
	setup() {
		...
		function addPro() {
			...
			var phone = document.getElementById("telinp").value;
			...
			axios
				...
				.then((resp) => {
					var loc = `${resp.data[0].state}-${resp.data[0].city}`;
					var seller_name = resp.data[0].allname;
					axios
						...
						.then((resp) => {
							if (resp.data.inserted) {
								nothide.value = false;
								inserted.value = true;
								notftext.value = "محصول شما ثبت شد";
							} else {
								if (resp.data.err == "blocked char") {
									...
									nothide.value = false;
									inserted.value = false;
									notftext.value = "ثبت محصول شما با مشکل مواجه شد";
								}
							}
							setTimeout(() => {
								nothide.value = true;
